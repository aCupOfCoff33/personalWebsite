...
const PhotoHeader = React.memo(function PhotoHeader({ image, overlayText }) {
  return (
    ...
    >
      ...
      {overlayText ? (
        <div
          className="absolute inset-0 bg-black/30 flex items-end pointer-events-none"
          style={{ willChange: "auto" }}
        >
          <h3 className="p-6 text-2xl md:text-4xl font-semibold text-white select-none">
            {overlayText}
          </h3>
        </div>
      ) : null}
...
const NoteSection = React.memo(function NoteSection({ section }) {
  switch (section.type) {
    case "heading": {
      const Tag =
        section.level === 1 ? "h2" : section.level === 3 ? "h4" : "h3";
      return (
        <Tag
          id={section.id}
    ...
    case "photoHeader":
      return (
        <PhotoHeader image={section.image} overlayText={section.overlayText} />
      );
    case "code":
      return (
        <Suspense
