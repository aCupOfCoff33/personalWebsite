import React from "react";
import BearEars from "../../../components/common/BearEars";

// BaseBear renders only the static visual paths for the bear (no <svg>, no state/logic)
// New design uses viewBox="0 0 68 68" coordinate system
const BaseBear = React.memo(() => (
  <>
    {/* Background circle */}
    <circle cx="34" cy="34" r="34" fill="#E7E9E8" />

    {/* Ears */}
    <BearEars />

    {/* Head - main brown circle */}
    <path
      d="M33.6406 14.75C47.1716 14.75 58.1406 24.6557 58.1406 36.875C58.1406 49.0943 47.1716 59 33.6406 59C20.1096 59 9.14062 49.0943 9.14062 36.875C9.14062 24.6557 20.1096 14.75 33.6406 14.75Z"
      fill="#B56A2B"
    />

    {/* Muzzle/snout area */}
    <path
      d="M34 34C40.9229 34 42.9999 40.7853 43 44.5322C43 51.6425 38.9076 53 34.0645 53C29.2215 52.9999 25 50.9637 25 44.5322C25.0001 40.7853 27.0771 34 34 34Z"
      fill="#CD853F"
    />

    {/* Mouth details - left side */}
    <path
      d="M30.4018 45.6937C30.3419 45.6937 30.042 45.5138 29.3225 44.7942C28.6029 44.0746 29.1426 43.7748 29.5024 43.7148C29.5623 43.9547 30.042 44.5783 31.4812 45.154C33.2801 45.8736 33.8198 44.0746 33.8198 44.7942C33.8198 45.5138 33.8198 45.6937 33.1002 46.0535C32.5246 46.3413 31.6611 46.1734 31.3013 46.0535L30.4018 45.6937Z"
      fill="#241E16"
      stroke="#281E11"
    />

    {/* Mouth details - right side */}
    <path
      d="M37.063 45.6937C37.123 45.6937 37.4228 45.5138 38.1424 44.7942C38.862 44.0746 38.3223 43.7748 37.9625 43.7148C37.9025 43.9547 37.4228 44.5783 35.9836 45.154C34.1847 45.8736 33.645 44.0746 33.645 44.7942C33.645 45.5138 33.645 45.6937 34.3646 46.0535C34.9403 46.3413 35.8038 46.1734 36.1635 46.0535L37.063 45.6937Z"
      fill="#241E16"
      stroke="#281E11"
    />

    {/* Nose vertical line */}
    <rect
      x="33.6399"
      y="38.676"
      width="0.359788"
      height="6.83598"
      fill="#241E16"
      stroke="#281E11"
      strokeWidth="0.359788"
    />

    {/* Nose oval */}
    <path
      d="M33.8203 36.8379C34.8324 36.8379 35.7253 37.0999 36.3496 37.4971C36.98 37.8982 37.2773 38.3912 37.2773 38.8564C37.2773 39.3217 36.9799 39.8147 36.3496 40.2158C35.7253 40.613 34.8324 40.8749 33.8203 40.875C32.808 40.875 31.9144 40.6131 31.29 40.2158C30.6597 39.8147 30.3623 39.3217 30.3623 38.8564C30.3623 38.3912 30.6597 37.8982 31.29 37.4971C31.9144 37.0998 32.808 36.8379 33.8203 36.8379Z"
      fill="#241E16"
      stroke="#281E11"
    />

    {/* Eyes are rendered by BearEyes for animations */}
  </>
));

BaseBear.displayName = "BaseBear";
export default BaseBear;
