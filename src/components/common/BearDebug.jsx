...
const BearDebug = () => {
  ...
  return (
    ...
    >
      <div>Type: {bearState.currentType}</div>
      <div>Previous: {bearState.previousType}</div>
      <div>Position: {bearState.itemPosition}</div>
      <div>Transitioning: {bearState.isTransitioning ? 'Yes' : 'No'}</div>
    </div>
  );
};
