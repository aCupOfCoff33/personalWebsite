...
import { vscDarkPlus as prismTheme } from 'react-syntax-highlighter/dist/esm/styles/prism';
...
export default function CodeBlock({ code, language = 'javascript', filename, showLineNumbers = true, renderHeader = true }) {
  const [ready, setReady] = React.useState(registered.has(language));
  const [copied, setCopied] = React.useState(false);

  const { cleanedCode, highlightedLines } = React.useMemo(() => parseHighlightedLines(code || ''), [code]);
  ...
  if (!ready) {
    return <pre className="text-sm text-neutral-300 p-4 bg-black/30 rounded-xl overflow-auto">Loading code…</pre>;
  }
